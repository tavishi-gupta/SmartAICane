...
// =========================================================================
// Object classifier (ArduTFLite) — added on top of the base CameraWebServer.
// Serves predictions at GET /classify as JSON: {"label":"...","confidence":0.87}
// =========================================================================
...
// Increase kTensorArenaSize if modelInit() fails.
static const int kTensorArenaSize = 200 * 1024;
static uint8_t *tensorArena = nullptr;
static bool classifierReady = false;

void classifier_init() {
  tensorArena = (uint8_t *)heap_caps_malloc(kTensorArenaSize, MALLOC_CAP_SPIRAM);
  if (!tensorArena) {
    log_e("Failed to allocate tensor arena in PSRAM");
    return;
  }
  classifierReady = modelInit(model_int8_tflite, tensorArena, kTensorArenaSize);
  if (!classifierReady) {
    log_e("Classifier model init failed — try increasing kTensorArenaSize");
  } else {
    log_i("Classifier ready");
  }
}
...
static esp_err_t classify_handler(httpd_req_t *req) {
  ...
  if (rgb_buf) {
    ok = fmt2rgb888(fb->buf, fb->len, fb->format, rgb_buf);
  }
